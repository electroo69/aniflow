...
export const Details = () => {
            ...
            try {
                ...
                if (isManga) {
                    const [itemRes, charRes, recRes] = await Promise.all([
                        jikanApi.getMangaById(id),
                        jikanApi.getMangaCharacters(id),
                        jikanApi.getMangaRecommendations(id)
                    ]);
                    setItem(itemRes.data);
                    setCharacters(charRes.data);
                    setRecommendations(recRes.data.map((r: any) => ({ ...r.entry, type: 'Manga' })));

                    // Redirect to correct SEO URL if needed
                    const correctSlug = generateSlug(itemRes.data.title_english || itemRes.data.title);
                    const expectedPath = `/manga/${correctSlug}-${id}`;
                    ...
                    // Redirect to correct SEO URL if needed
                    const correctSlug = generateSlug(itemRes.data.title_english || itemRes.data.title);
                    const expectedPath = `/anime/${correctSlug}-${id}`;
                    if (location.pathname !== expectedPath) {
                        navigate(expectedPath, { replace: true });
                    }
                }
    ...
    }, [id, isManga, navigate, location.pathname]);
    ...
    const jsonLdData = useMemo(() => {
        ...
        const canonicalUrl = `${BASE_URL}/${isManga ? 'manga' : 'anime'}/${slug}-${item.mal_id}`;

        // Generate breadcrumb
        const breadcrumb = generateBreadcrumbJsonLd([
            { name: 'Home', url: BASE_URL },
            { name: isManga ? 'Manga' : 'Anime', url: `${BASE_URL}/${isManga ? 'manga' : 'anime'}` },
            { name: displayTitle, url: canonicalUrl }
        ]);

        // Generate item-specific JSON-LD
        const itemJsonLd = isManga
            ? generateMangaJsonLd(item as Manga)
            : generateAnimeJsonLd(item as Anime);

        return [breadcrumb, itemJsonLd];
    }, [item, isManga]);
                ...
                <div className="absolute bottom-0 left-0 w-full p-6 md:p-10 flex flex-col md:flex-row gap-8 items-end">
                    ...
                    <div className="flex-1 mb-2">
                        <div className="flex flex-wrap gap-2 mb-3">
                            ...
                            <span className={`px-2 py-0.5 rounded text-xs font-bold uppercase tracking-wider ${(item as Anime).airing || (item as Manga).publishing
                ...
                <div className="space-y-6">
                    ...
                    <div className="flex justify-center">
                        <AdBanner
                            width={300}
                            height={250}
                            dataKey="edcee0617b46e89591f39e8a7b1e13a9"
                            className="my-6"
                        />
                    </div>
                </div>
                    ...
                    {trailerUrl && (
                        <div className="space-y-4">
                            ...
                            <div className="aspect-video rounded-xl overflow-hidden shadow-lg bg-black">
                                <iframe
                                    src={`${trailerUrl}?autoplay=0&mute=0&rel=0`}
                                    title="Trailer"
